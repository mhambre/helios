...
daemon-debug d-debug dd:
	...
	@echo "[build] $(DAEMON_CRATE) built successfully in debug mode: $(call target_dir_for_component,$(DAEMON_COMPONENT))/debug/$(DAEMON_BIN)"
...
daemon-release d-release dr:
	...
	@echo "[build] $(DAEMON_CRATE) built successfully in release mode: $(call target_dir_for_component,$(DAEMON_COMPONENT))/release/$(DAEMON_BIN)"
...
daemon-gdb dgdb: daemon-debug
	@if [ "$(call component_level,$(DAEMON_COMPONENT))" != "high" ]; then \
		echo "[err] $(DAEMON_COMPONENT) is currently low-level; move it to high-level first (make set-high c=$(DAEMON_COMPONENT))."; \
		exit 1; \
	fi
