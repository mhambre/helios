...
ctl-debug cd:
	...
	@echo "[build] $(CONTROL_CRATE) built successfully in debug mode: $(call target_dir_for_component,$(CONTROL_COMPONENT))/debug/$(CONTROL_BIN)"

# Build control in release mode
.PHONY: ctl-release cr
ctl-release cr:
	$(call cargo_build_component,$(CONTROL_COMPONENT),$(CONTROL_CRATE),release)
	@echo "[build] $(CONTROL_CRATE) built successfully in release mode: $(call target_dir_for_component,$(CONTROL_COMPONENT))/release/$(CONTROL_BIN)"
...
ctl-gdb cgdb: ctl-debug
	@if [ "$(call component_level,$(CONTROL_COMPONENT))" != "high" ]; then \
		echo "[err] $(CONTROL_COMPONENT) is currently low-level; move it to high-level first (make set-high c=$(CONTROL_COMPONENT))."; \
		exit 1; \
	fi
