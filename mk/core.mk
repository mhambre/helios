...
kernel-debug kd:
	...
	grub-file --is-x86-multiboot2 $(call target_dir_for_component,$(CORE_COMPONENT))/debug/$(CORE_BIN) && echo "Kernel binary is Multiboot2 compliant" || (echo "Kernel binary is not Multiboot2 compliant" && exit 1)
	@echo "Kernel built successfully in debug mode: $(call target_dir_for_component,$(CORE_COMPONENT))/debug/$(CORE_BIN)"

# Build kernel in release mode
.PHONY: kernel-release kr
kernel-release kr:
	$(call cargo_build_component,$(CORE_COMPONENT),$(CORE_CRATE),release)
	grub-file --is-x86-multiboot2 $(call target_dir_for_component,$(CORE_COMPONENT))/release/$(CORE_BIN) && echo "Kernel binary is Multiboot2 compliant" || (echo "Kernel binary is not Multiboot2 compliant" && exit 1)
	@echo "Kernel built successfully in release mode: $(call target_dir_for_component,$(CORE_COMPONENT))/release/$(CORE_BIN)"
...
kernel-qemu-gdb kqgdb: debug-iso-serial
	@echo "QEMU waiting for GDB on tcp::$(QEMU_GDB_PORT)"
